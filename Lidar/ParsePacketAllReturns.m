function [Timestamp,RangeBearing,Intensity] = ParsePacket3Returns(TCPhandle,PacketsPerScan)
...
% status packet [1] 6621-6632
 ...
 %total bytes per packet 4*4+1*4+50*(2*2+4*3*8+1*3*8+1*8)+4*2+2*2 = 6632 bytes 
    ...
    %-----------------------------------------
    % init vars
	Intensity=zeros(50*PackeCount,3*8); 
	RangeBearing=zeros(50*PackeCount,1+3*8);
...
%         MessageSize = typecast(flip(Packet(5+PacketInd:8+PacketInd),2),'uint32');
        ...
        for i2=1:50 %process 132 bytes per loop iteration
            ...
            %uint32 Distance(3*8) [10 micrometers]
            RangeBearing(i*50+i2,2:25) = double(typecast(flip(Packet(132*(i2-1)+25+PacketInd:132*(i2-1)+120+PacketInd),2),'uint32')).*10^-5;%distance to meter
            %uint8 Intensities(3*8) [0-255]
            Intensity(i*50+i2,1:24) = Packet(132*(i2-1)+121+PacketInd:132*(i2-1)+144+PacketInd);
