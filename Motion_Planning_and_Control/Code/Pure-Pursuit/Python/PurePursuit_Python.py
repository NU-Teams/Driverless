...
def main():
    ...
    T = 100000.0  # max simulation time
    ...
    while T >= simTime:  #may need to get rid of the second criteria for multiple laps
        # starting time
        start = time.time()
        ...
        if (calcTime != 0.0):
            print(f"Runtime of the program is {round((calcTime)*1000,3)}s")
            nTime = calcTime
        ...
        if show_animation:  # pragma: no cover
            ...
            plt.plot(cx, cy, "-r", label="course", marker=".",markersize=10)
            plt.plot(states.x, states.y, "-b", label="trajectory")
            plt.plot(cx[target_ind], cy[target_ind], "xg", label="target")
            plt.axis("equal")
            plt.grid(True)
            plt.title("Speed[km/h]:" + str(state.v * 3.6)[:4]+"  CalcTime[ms]:"+str(round((nTime)*1000,3)))
            plt.pause(0.001)
