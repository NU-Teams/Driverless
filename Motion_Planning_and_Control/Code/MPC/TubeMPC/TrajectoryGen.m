...
trajectoryInfo = [0,    0,-5,0,         0,0,0; ... % Initial position
                  ...
                  10.5,    3,-13,0,        0,0,0; ...
                  11.4,  0,-8,0,         0,0,0; ...
                  11.7,    0,-5,0,         180,0,0];    % Final position
elseif course == 1
%% square Path
...
trajectoryInfo = [0,    0,0,0,         0,0,0; ... % Initial position
                  0.1,  r,-r,0,      0,0,0; ...
                  0.2,  2*r,-2*r,0,         0,0,0;
                  0.3,  4*r,-4*r,0,         0,0,0;];    % Final position
end
...
%% interp
%3ji
pathpoints = trajectoryInfo(:,2:3)
interp1(1:length(pathpoints),pathpoints,linspace(1,50)
%% plot points 
figure(1)
...
while ~isDone(trajectory)
   [currentPosition,orientationLog(count) V a aV] = trajectory();    
   plot(currentPosition(1),currentPosition(2),'bo')
   xpos = [xpos,currentPosition(1)];
   ypos = [ypos,currentPosition(2)];
   Velocities = [Velocities,sqrt(V(1)^2+V(2)^2)];
   angularVelocity = [angularVelocity,aV(3)];
...
RefrencePath = [xpos;ypos;Velocities;angularVelocity];
Map = [xpos;ypos];
save('RefrencePath.mat','RefrencePath')
save('xpos.mat','xpos')
save('ypos.mat','ypos')
