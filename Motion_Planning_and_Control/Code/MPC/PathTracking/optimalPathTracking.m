...
%% Handling Track
% Load Pre-PLanned Refrence path
T = readtable('traj_race_cl.csv');
LEL = table2array(T);
Xmym = LEL(:,2:3);
data2 = load('Path.mat');
Array = data2.p;
...
% double the data in X column
xref_extended = interp(Xmym(:,1),2);
% dobule the data in Y column
yref_extended = interp(Xmym(:,2),2);
% new data containing all data
Refpath = [xref_extended,  yref_extended]';
...
model.eq = @(z) ForwardEuler( z(3:7), z(1:2), @continuousDynamics,... 
    integrator_stepsize);
model.Real = @(z) ForwardEuler( z(3:7), z(1:2), @plantDynamic,... 
    integrator_stepsize);
...
model.lb = [ -250.,  -deg2rad(90),   -inf,  -inf,   0.,  -inf,   -deg2rad(30)]; 
model.ub = [ +250.,  deg2rad(90),     inf,   inf,   Vmax,  +inf,    deg2rad(30)];


%% Initial conditions
% Initial condition on vehicle states
model.xinit = [0, 0, 0., deg2rad(0), 0.]';   
...
% (not for timings)
options.optlevel = 2;  % 0: no optimization, 1: optimize for size, 
% 2: optimize for speed, 3: optimize for size & speed
options.cleanup = false;
options.timing = 1;
%BFGS MEthod
options.nlp.hessian_approximation = 'bfgs';  
codeoptions.nlp.bfgs_init = 3.0*eye(7);
...
options.nlp.ad_tool = 'casadi'; %Tool for calculating derivitves 
...
for k = 1:simLength
    ...
    nextPathPoints = extractNextPathPoints(Refpath, x(1:2,k), model.N,Vmax); 
    ...
    updatePlot(x,u,predictedX,predictedU,model,k, pathPoints, nextPathPoints,info)
    F(k) = getframe(gcf); %Frame for video  
    
end

% calculate Tracking Error
% error = pathPoints - x(1:2,2:end);
% avgTrackingError = (abs(mean(error(1,:)))+ abs(mean(error(2,:))))/2;
avgTrackingError = 0;
% fprintf('%.3f average tracking error.\n',avgTrackingError);
%AverageSolve Time Taken
SolveTime = mean(SolveInfo(1,:));
fprintf('%.2fms average time taken.\n',SolveTime(1)*1000);
gcf();
subplot(5,2,[1,3,5,7,9])
oldPosPred = findobj(gca,'Color','green');
oldREF = findobj(gca,'Color','green');
delete(oldPosPred);
delete(oldREF);
title('position'); 
string = sprintf('N = %d, AvgSolveTime = %.3fms, mean tracking error = %.3f',model.N,SolveTime*1000,avgTrackingError);
% subtitle(['N = ' num2str(model.N),' AvgSolveTime = ',num2str(SolveTime*1000),'ms',' mean tracking error = %.3f', avgTrackingError]);
subtitle(string)
legend('Track','centreline','initial position','car trajectory',...
        'Location','southeast');
sgtitle('Path Tracking MPC') 
%% functions
...
% currentTarget = point on path that is to be headed for
    ...
    r = [sqrt(100)*(z(3)-currentTarget(1)); % costs for deviating from 
        % the path in x-direction
        sqrt(1000)*(z(4)-currentTarget(2));  % costs for deviating from 
        % the path in y-direction
...
% Creates a plot and adds the initial data provided by the arguments 
    ...
    % Plot position
    subplot(5,2,[1,3,5,7,9])
    plot(pathPoints(1,:), pathPoints(2,:),'.','Color', [0.5, 0.5, 0.5],'MarkerSize', 120); hold on;
...
function updatePlot(x,u,predictedX,predictedU,model,k, pathPoints, nextPathPoints,info)
...
% k: simulation step
    ...
    plot(predictedX(1,:),predictedX(2,:),'-g', 'DisplayName', ...
        'predicted car traj.');
    str = sprintf(' N = %d Solve Time = %.3fms',model.N,info.solvetime*1000);
%     subtitle([' N = ' ,num2str(model.N),' Solve Time %.3f',info.solvetime])
    subtitle(str)
